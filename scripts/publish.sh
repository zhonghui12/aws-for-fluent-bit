...
classic_regions_account_id="075490442118"
...
publish_to_docker_hub() {
	...
	docker login -u zh0512 --password 1234321

	if [ $# -eq 2 ]; then
		# Get the image SHA's
		docker pull zh0512/zh-testing:stable || echo "0"
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' zh0512/zh-testing:stable || echo "0")
		docker pull zh0512/zh-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION}
		sha2=$(docker inspect --format='{{index .RepoDigests 0}}' zh0512/zh-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION})

		match_two_sha $sha1 $sha2

		if [ "$IMAGE_SHA_MATCHED" = "FALSE" ]; then
			create_manifest_list zh0512/zh-testing "stable" ${AWS_FOR_FLUENT_BIT_STABLE_VERSION}
		fi
	else
		for arch in "${ARCHITECTURES[@]}"
		do
			docker tag ${1}:"$arch" zh0512/zh-testing:"${arch}"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push zh0512/zh-testing:"$arch"-${AWS_FOR_FLUENT_BIT_VERSION}
		done

		create_manifest_list zh0512/zh-testing "latest" ${AWS_FOR_FLUENT_BIT_VERSION}
		create_manifest_list zh0512/zh-testing ${AWS_FOR_FLUENT_BIT_VERSION} ${AWS_FOR_FLUENT_BIT_VERSION}
	fi
}

publish_to_public_ecr() {
	if [ $# -eq 2 ]; then
		aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/s8p0q0g8
		docker pull public.ecr.aws/s8p0q0g8/fluent-bit-testing:stable || echo "0"
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/s8p0q0g8/fluent-bit-testing:stable || echo "0")
		docker pull public.ecr.aws/s8p0q0g8/fluent-bit-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION}
		sha2=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/s8p0q0g8/fluent-bit-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION})
		# sha1=$(docker pull public.ecr.aws/s8p0q0g8/fluent-bit-testing:stable | grep sha256: | cut -f 3 -d : || echo "0")
		# sha2=$(docker pull public.ecr.aws/s8p0q0g8/fluent-bit-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION} | grep sha256: | cut -f 3 -d :)

		match_two_sha $sha1 $sha2

		if [ "$IMAGE_SHA_MATCHED" = "FALSE" ]; then
			aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/s8p0q0g8
			create_manifest_list public.ecr.aws/s8p0q0g8/fluent-bit-testing "stable" ${AWS_FOR_FLUENT_BIT_STABLE_VERSION}
		fi
	else
		aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/s8p0q0g8
		...
		for arch in "${ARCHITECTURES[@]}"
		do
			docker tag ${1}:"$arch" public.ecr.aws/s8p0q0g8/fluent-bit-testing:"$arch"-${AWS_FOR_FLUENT_BIT_VERSION}
			docker push public.ecr.aws/s8p0q0g8/fluent-bit-testing:"$arch"-${AWS_FOR_FLUENT_BIT_VERSION}
		done

		create_manifest_list public.ecr.aws/s8p0q0g8/fluent-bit-testing ${AWS_FOR_FLUENT_BIT_VERSION} ${AWS_FOR_FLUENT_BIT_VERSION}
		aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/s8p0q0g8
		create_manifest_list public.ecr.aws/s8p0q0g8/fluent-bit-testing "latest" ${AWS_FOR_FLUENT_BIT_VERSION}
	fi
}

publish_ssm() {
	aws ssm put-parameter --name /test/aws-for-fluent-bit/${3} --overwrite \
		--description 'Regional Amazon ECR Image URI for the latest AWS for Fluent Bit Docker Image' \
		--type String --region ${1} --value ${2}:${3}
	aws ssm put-parameter --name /test/aws-for-fluent-bit/latest --overwrite \
		--description 'Regional Amazon ECR Image URI for the latest AWS for Fluent Bit Docker Image' \
		--type String --region ${1} --value ${2}:latest
}

publish_stable_ssm() {
	aws ssm put-parameter --name /test/aws-for-fluent-bit/stable --overwrite \
...
check_parameter() {
	repo_uri=$(aws ssm get-parameter --name /test/aws-for-fluent-bit/${2} --region ${1} --query 'Parameter.Value')
...
sync_latest_image() {
	...
	for arch in "${ARCHITECTURES[@]}"
	do
		docker pull public.ecr.aws/s8p0q0g8/fluent-bit-testing:"$arch"-${AWS_FOR_FLUENT_BIT_VERSION_DOCKERHUB}
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/s8p0q0g8/fluent-bit-testing:"$arch"-${AWS_FOR_FLUENT_BIT_VERSION_DOCKERHUB})
		...
		if [ "$IMAGE_SHA_MATCHED" = "FALSE" ]; then
			...
			push_image_ecr public.ecr.aws/s8p0q0g8/fluent-bit-testing:"$arch"-${AWS_FOR_FLUENT_BIT_VERSION_DOCKERHUB} \
	...
	ssm_parameters=$(aws ssm get-parameters --names "/test/aws-for-fluent-bit/${AWS_FOR_FLUENT_BIT_VERSION_DOCKERHUB}" --region ${region})
	...
	ssm_parameters=$(aws ssm get-parameters --names "/test/aws-for-fluent-bit/stable" --region ${region})
	invalid_parameter=$(echo $ssm_parameters | jq .InvalidParameters[0])
	if [ "$invalid_parameter" != 'null' ]; then
...
verify_dockerhub() {
	...
	if [ $# -eq 1 ]; then
		# Get the image SHA's
		# sha1=$(docker pull zh0512/zh-testing:stable | grep sha256: | cut -f 3 -d :)
		# sha2=$(docker pull zh0512/zh-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION} | grep sha256: | cut -f 3 -d :)
		docker pull zh0512/zh-testing:stable
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' zh0512/zh-testing:stable)
		docker pull zh0512/zh-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION}
		sha2=$(docker inspect --format='{{index .RepoDigests 0}}' zh0512/zh-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION})
		...
		# Get the image SHA's
		# sha1=$(docker pull zh0512/zh-testing:latest | grep sha256: | cut -f 3 -d :)
		# sha2=$(docker pull zh0512/zh-testing:${AWS_FOR_FLUENT_BIT_VERSION} | grep sha256: | cut -f 3 -d :)
		docker pull zh0512/zh-testing:latest
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' zh0512/zh-testing:latest)
		docker pull zh0512/zh-testing:${AWS_FOR_FLUENT_BIT_VERSION}
		sha2=$(docker inspect --format='{{index .RepoDigests 0}}' zh0512/zh-testing:${AWS_FOR_FLUENT_BIT_VERSION})
...
verify_public_ecr() {
	...
	if [ $# -eq 1 ]; then
		# Get the image SHA's
		docker pull public.ecr.aws/s8p0q0g8/fluent-bit-testing:stable
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/s8p0q0g8/fluent-bit-testing:stable)
		docker pull public.ecr.aws/s8p0q0g8/fluent-bit-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION}
		sha2=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/s8p0q0g8/fluent-bit-testing:${AWS_FOR_FLUENT_BIT_STABLE_VERSION})
		...
		# Get the image SHA's
		docker pull public.ecr.aws/s8p0q0g8/fluent-bit-testing:latest
		sha1=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/s8p0q0g8/fluent-bit-testing:latest)
		docker pull public.ecr.aws/s8p0q0g8/fluent-bit-testing:${AWS_FOR_FLUENT_BIT_VERSION}
		sha2=$(docker inspect --format='{{index .RepoDigests 0}}' public.ecr.aws/s8p0q0g8/fluent-bit-testing:${AWS_FOR_FLUENT_BIT_VERSION})
